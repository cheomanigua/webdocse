...
Changes here will affect the autogenerated Taxonomy pages.
...
| Need | Choose |
|------|--------|
| **Same HTML wrapper for every page** | `layouts/baseof.html` + `{{ block "main" . }}{{ end }}` |
| **Header / Footer / Nav** | `_partials/` |
| **Reusable UI component that may contain logic** | `_partials/` (or a shortcode if authors need to drop it into markdown) |
| **Content authors should type &#123;&#123;< youtube id >&#125;&#125;** | `_shortcodes/youtube.html` |
| **Content authors should type `{{< youtube oayOen6OOXw >}}`** | `_shortcodes/youtube.html` |
| **A one-off landing page with custom layout** | Create `layouts/pages/single.html` **or** put a `layout: special` front-matter key |
| **Blog list page** | `layouts/list.html` (or `layouts/posts/list.html`) |
| **All other pages (about, contact, docs…)** | Fall back to `single.html` |
...
*`about.md` → `single.html`*  
*`posts/*.md` → `posts/list.html` (archive) + `single.html` (article)*  
...
Use **_partials** for DRY HTML, **_shortcodes** for author-friendly markdown, and **specific layout folders** (`posts/`, `docs/`, etc.) when you need a custom look for a particular section. Happy Hugo-ing!
